<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="周次" prop="zhouci">
          <el-input v-model="ruleForm.zhouci" 
              placeholder="周次" clearable  :readonly="ro.zhouci"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="周次" prop="zhouci">
              <el-input v-model="ruleForm.zhouci" 
                placeholder="周次" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="次数" prop="cishu">
          <el-input v-model="ruleForm.cishu" 
              placeholder="次数" clearable  :readonly="ro.cishu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="次数" prop="cishu">
              <el-input v-model="ruleForm.cishu" 
                placeholder="次数" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      this.$http({
		...
		if(json.jiaoshixingming!=''&&json.jiaoshixingming){
          ...
          this.$message.error(data.msg);
            ...
            this.kaoqinleixingOptions = "签到,旷课,迟到,事假,病假".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `kaoqintongji/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `kaoqintongji/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.kaoqintongjiCrossAddOrUpdateFlag = false;
						 this.parent.search();
